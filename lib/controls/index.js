...
import { createMultiMaterialObject } from '../threejs/jsm/utils/SceneUtils.js';
...
const basicType = {
    ...
    detail: getMeshValue([0, 5, 1], 'detail'),
    size: getMeshValue([0, 10, 1], 'size'),
    curveSegments: getMeshValue([1, 30, 1], 'curveSegments'),
    bevelEnabled: getMeshValue([], 'bevelEnabled'),
    bevelThickness: getMeshValue([1, 30, 1], 'bevelThickness'),
    bevelSize: getMeshValue([1, 30, 1], 'bevelSize'),
    bevelOffset: getMeshValue([0, 5, 1], 'bevelOffset'),
    bevelSegments: getMeshValue([1, 30, 1], 'bevelSegments'),
}
...
function createMaterial (geometry) {
    ...
    return new createMultiMaterialObject(geometry, [
        lambert,
        basic
    ]);
}
...
function removeAndAdd (item, value, camera, mesh, scene, controls) {
    ...
    if (isFont(item)) {
        console.log("类型", THREE[item.type], item.type)
        mesh.pointer = createMaterial(new TextGeometry('THREE', Object.assign(textOptions, controls)));
    } else {
...
function getMeshValue (extend, name) {
    return {
        extends: extend,
        getValue: (item, camera, mesh) => {
            if (isFont(item) && textOptions[name] !== undefined) {
                return textOptions[name]
            } else {
                return mesh.children[0].geometry.parameters[name]
            }
        },
...
const itemType = {
    ...
    TextGeometry: ['size', 'height', 'curveSegments', 'bevelEnabled', 'bevelThickness', 'bevelSize', 'bevelOffset', 'bevelSegments']
}

export function initControls (item, camera, mesh, scene, options) {
    textOptions = options;
